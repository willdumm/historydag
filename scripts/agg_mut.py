...
def summarize(dagpath, treedir, outfile, csv_data, print_header):
    ...
    data = []
    data.append(("beforeCollapseNTrees", dag.count_trees()))
    data.append(("beforeCollapseNNodes", len(list(dag.preorder()))))
    wc = dag.weight_count(edge_weight_func=dist)
    data.append(("beforeCollapseMaxPars", max(wc.keys())))
    data.append(("beforeCollapseMinPars", min(wc.keys())))
    dag.add_all_allowed_edges()
    dag.trim_optimal_weight(edge_weight_func=dist)
    dag.convert_to_collapsed()
    data.append(("nTrees", dag.count_trees()))
    wc = dag.weight_count(edge_weight_func=dist)
    data.append(("parsimonyScore", min(wc.keys())))
    data.append(("avgNodeParents", dag.internal_avg_parents()))
    data.append(("nNodes", len(list(dag.preorder()))))
    data.append(("nEdges", sum(len(list(n.children())) for n in dag.preorder())))
    data.append(("nLeaves", len([n for n in dag.preorder() if n.is_leaf()])))
    node_counts = dag.weight_count(edge_weight_func=lambda n1, n2: 1)
    data.append(("treeMinNNodes", min(node_counts.keys())))
    data.append(("treeMaxNNodes", max(node_counts.keys())))
    data.append(("nNodesUnlabeled", len(list(dag.unlabel().preorder()))))
    data.append(("nTreeRoots", len(list(dag.dagroot.children()))))
    if treedir:
        treepath = Path(treedir)
        treefiles = list(treepath.glob('*.pb'))
        wc = count_parsimony(treefiles)
        data.append(("nInputTrees", len(treefiles)))
        data.append(("uniqueTreesIn", count_unique(treefiles)))
        data.append(("inputMinPars", min(wc.keys())))
        data.append(("inputMaxPars", max(wc.keys())))
    outstring = ''
    if csv_data:
        if print_header:
