...
def test_parsimony():
    ...
    def parsimony(tree):
        ...
        return sum(dagutils.wrapped_hamming_distance(list(node.parents)[0].label, node.label)
...
def test_collapsed_counts():
    ...
    _testfactory(lambda dag: dag.weight_count(edge_weight_func=dagutils.access_nodefield_default('sequence', False)(lambda s1, s2: s1 == s2)), uncollapsed)
...
def test_count_trees_expanded():
    for dag in dags + cdags:
        ndag = dag.copy()
        ndag.explode_nodes()
        assert dag.count_trees(expand_func=dagutils.sequence_resolutions) == ndag.count_trees()
